...
s32 act_idle(struct MarioState *m) {
    ...
    if (m->actionArg & 1) {
        ...
        if (is_anim_at_end(m)) {
            // Fall asleep after 1 head-turning cycle.
            ...
            // and that he's gone through a cycle before sleeping.
            ...
            if (++m->actionState == 3) {
                ...
                if (deltaYOfFloorBehindMario < -24.0f || 24.0f < deltaYOfFloorBehindMario || m->floor->flags & SURFACE_FLAG_DYNAMIC) {
                    ...
                    // If Mario hasn't completed a full head-turn, stay idle instead of falling asleep.
                    m->actionTimer++;
                    if (m->actionTimer < 1) {
                        m->actionState = 0;
                    }
...
s32 act_sleeping(struct MarioState *m) {
    ...
    switch (m->actionState) {
        ...
        case 1:
            set_mario_animation(m, MARIO_ANIM_SLEEP_START_LYING);
...
s32 act_first_person(struct MarioState *m) {
    ...
    if (m->floor->type == SURFACE_LOOK_UP_WARP
        //&& save_file_get_total_star_count(gCurrSaveFileNum - 1, COURSE_MIN - 1, COURSE_MAX - 1) >= 10) { make it 0 for testing
